...
int format_analysis(Sequence *new_seq,int input)
{
	char 	   *block;
...
///////////         analyse fasta format         /////////////////////////////
	if(block[0] == '>' ){
		...
		sprintf(new_seq->locus,"%s_F%d",locus,SEQUENCE_MARKER++);	
		for(m=0;m<200;m++){
			...
			new_seq->description[m] = block[n++];
			if(isalnum(new_seq->description[m]) == NULL 
				&& new_seq->description[m]!=',' && new_seq->description[m]!='.' && new_seq->description[m]!='\n')
				new_seq->description[m] = ' ';
		
			if(new_seq->description[m] == '\n'){
				new_seq->description[m] = '\0';
				break;
			}
		}
		sprintf(new_seq->source,"not_specific");
		for(;n<1000;n++){
			if(	block[n] == 'A' || block[n] == 'C' || block[n] == 'G' || block[n] == 'T' ||
				block[n] == 'a' || block[n] == 'c' || block[n] == 'g' || block[n] == 't'){
				new_seq->data_startpoint = n;
				break;
			}
		}
	}	
////////////        analyse genbank foramt       ////////////////////////////
		...
		for(;n<250;n++){if(block[n] != ' ') break;}
	 	for(m=0;m<200;m++){
			...
			if(isalnum(new_seq->description[m]) == NULL && new_seq->description[m]!=',' && new_seq->description[m]!='.')
				new_seq->description[m] = ' ';
		}
////////////  appended in 2004.4.19 to recode information of genome source //////////////////
		for(;n<5000;n++){
					   ...
					   && block[n+3] == 'R' && block[n+4] == 'C' && block[n+5] == 'E') 
			{
				...
				for(;;){if(block[++n] != ' ') break;}
				for(m=0;m<40;m++){
					new_seq->source[m] = block[n++];
					if(isalnum(new_seq->source[m]) == NULL)				
						new_seq->source[m] = '_';
					if(block[n] == '.' || (block[n+1] == 'O' && block[n+2] == 'R' 
						&& block[n+3] == 'G')) 
						break;
				}
				new_seq->source[m+1]= '\0';
				break;
			}
		}
////////////   by author Shen Junjie ///////////////////////////////////////////////////////////
		new_seq->data_startpoint = 0;
		for(;n<MALLOC_SIZE-5;n++){
			if(block[n] == 'O' && block[n+1] == 'R' && block[n+2] == 'I' 
				&& block[n+3] == 'G' && block[n+4] == 'I' && block[n+5] == 'N'){
				new_seq->data_startpoint = n+6;
				break;
			}
		}
		if(new_seq->data_startpoint == 0){
			for(m=1;m<100;m++){
				...
				for(n = 0;n<MALLOC_SIZE-5;n++){
					if(block[n] == 'O' && block[n+1] == 'R' && block[n+2] == 'I'
						&& block[n+3] == 'G' && block[n+4] == 'I' && block[n+5] == 'N'){
						new_seq->data_startpoint = n+6 + m*(MALLOC_SIZE*sizeof(char)-5);
						break;
					}
				}
				if(new_seq->data_startpoint != 0)
...
Sequence* data_process(char *file)
{
	...
	new_seq = malloc(sizeof(Sequence));
	new_seq->last = new_seq->next = NULL;
	for(n = 0;n<200;n++){
		new_seq->filepath[n] = file[n];
		if(new_seq->filepath[n] == '\0')
			break;
	}
	
//// format analysis is an important process before reading data ////
	format = format_analysis(new_seq,input);
/////////////////////////////////////////////////////////////////////
...
////////////  reading sequence data   ////////////////////////////////////	
	lseek(input,(off_t)(new_seq->data_startpoint),SEEK_SET);	
	new_seq->data[new_seq->datalist] = (char*)malloc(MALLOC_SIZE*sizeof(char));
	while((nread = read(input,block,sizeof(block))) > 0 ){
		for(n = 0;n<nread;n++){
			switch (block[n]){
				case 'g':
				case 'G':	new_seq->data[new_seq->datalist][buffer_point/4]&exp_table1[buffer_point%4];
					new_seq->data[new_seq->datalist][buffer_point/4]= new_seq->data[new_seq->datalist][buffer_point/4]+(GUANINE*exp_table2[buffer_point%4]);
							buffer_point++;
							g++;
							break;
				case 'c':	
				case 'C':	new_seq->data[new_seq->datalist][buffer_point/4]&exp_table1[buffer_point%4];
					new_seq->data[new_seq->datalist][buffer_point/4]= new_seq->data[new_seq->datalist][buffer_point/4]+(CYTOSINE*exp_table2[buffer_point%4]);
							buffer_point++;
							c++;
							break;
				case 'a':	
				case 'A':	new_seq->data[new_seq->datalist][buffer_point/4]&exp_table1[buffer_point%4];
							new_seq->data[new_seq->datalist][buffer_point/4] = new_seq->data[new_seq->datalist][buffer_point/4]+(ADENINE*exp_table2[buffer_point%4]);
							buffer_point++;
							a++;
							break;
				case 't':	
				case 'T':	new_seq->data[new_seq->datalist][buffer_point/4]&exp_table1[buffer_point%4];
					new_seq->data[new_seq->datalist][buffer_point/4]= new_seq->data[new_seq->datalist][buffer_point/4]+(THYMINE*exp_table2[buffer_point%4]);
							buffer_point++;
							t++;
							break;
	...
	new_seq->amount_basepairs 	= 	bps = a+c+g+t;				
	new_seq->amount_adenine		=	a;
	new_seq->amount_cytosine	=	c;
	new_seq->amount_guanine		=	g;
	new_seq->amount_thymine		=	t;
...
////////		data process     //////////////////////////////////////////////
	Sequence *new_p = data_process(filepath);
	if(new_p == -2){
...
///////////////////////////////////////////////////////////////////////////////
	gchar *text[4];
	text[0] = new_p->locus;
    text[1] = new_p->description;
    text[2] = new_p->format;
	text[3] = new_p->filepath;
	...
	if(HEAD_PORTRAIT == NULL)
		...
		for(;;){
			if(temp->next!=NULL)
				...
				temp->next = new_portrait;
				new_portrait->last = temp;
				break;
			}
		}
	}
				...
				for(;;temp_portrait = (Portrait*)temp_portrait->next){
					if(((Portrait*)portrait)->marker == ((Portrait*)temp_portrait->next)->marker)
					{
						...
						if(temp_portrait->next!= NULL)
							((Portrait*)temp_portrait->next)->last = temp_portrait;
						break;	
					}
				}
			}
